...
(function ($) {
    ...
    mirandaDoc = {
        ...
        myCustomRoomSlider() {
            $(".rooms-slider").slick({
                ...
                arrows: true,
                prevArrow:
                    '<button class="slick-prev"><i class="fal fa-chevron-left"></i></button>',
                nextArrow:
        ...
        initBannerSlider(container = null) {
            let bannerSliderOne = container
                ? $(container).find("#bannerSlider")
                : $("#bannerSlider");

            if (
                !bannerSliderOne.length ||
                bannerSliderOne.hasClass("slick-initialized")
            ) {
                return;
            }

            bannerSliderOne.on("init", function () {
                let $firstAnimatingElements = $(
        ...
        initRoomSlider(container = null) {
            let sliderImg = container
                    ? $(container).find(".rooms-slider-one")
                    : $(".rooms-slider-one"),
                sliderContent = container
                    ? $(container).find(".room-content-slider")
                    : $(".room-content-slider"),
                countStatus = container
                    ? $(container).find(".slider-count")
                    : $(".slider-count"),
                countBig = container
                    ? $(container).find(".slider-count-big")
                    : $(".slider-count-big");

            if (!sliderImg.length || sliderImg.hasClass("slick-initialized")) {
                return;
            }
        ...
        initTestimonialSlider(container = null) {
            let tslider = container
                ? $(container).find(".testimonial-slider")
                : $(".testimonial-slider");

            if (!tslider.length || tslider.hasClass("slick-initialized")) {
                return;
            }
            tslider.slick({
                rtl: isRTL,
        ...
        //===== 05. Latest Post Slider
        latestPostlider() {
            this.initLatestPostSlider();
        },

        initLatestPostSlider(container = null) {
            let tslider = container
                ? $(container).find(".latest-post-slider")
                : $(".latest-post-slider");
            let arrows = container
                ? $(container).find(".latest-post-arrow")
                : $(".latest-post-arrow");
        ...
        instaFeedSlider() {
            ...
            if (jQuery().magnificPopup) {
                $(".instagram-slider").each(function () {
                    ...
                    let additionalImages = $(".image a.insta-popup").not(
        ...
        initWow(container = null) {
            if (typeof WOW !== "undefined") {
                if (container) {
                    // Reinitialize only for new elements
                    $(container)
                        .find(".wow")
                        .each(function () {
                            new WOW().init();
            ...
            if ($(".date-picker").length > 0) {
                ...
                $(document).on("change", "#arrival-date", function () {
                    ...
                    $("#departure-date")
                        ...
                        .datepicker("option", "startDate", date);
                });
            }
        },
        ...
        initPopupVideo(container = null) {
            if (jQuery().magnificPopup) {
                let popupVideos = container
                    ? $(container).find(".popup-video")
                    : $(".popup-video");

                popupVideos.each(function () {
                    if (!$(this).hasClass("mfp-initialized")) {
                        $(this).magnificPopup({
                            type: "iframe",
                        });
                        $(this).addClass("mfp-initialized");
                    }
                });
            }
        },

        initPopupGallery(container = null) {
            if (jQuery().magnificPopup) {
                let popupImages = container
                    ? $(container).find(".gallery-loop .popup-image")
                    : $(".gallery-loop .popup-image");
    ...
    $(document).ready(function () {
        $(".service-item").on("change", function () {
            ...
            let $selectedPaymentMethod = $(document)
                .find(
                    '.payment-checkout-form .list_payment_method input[name="payment_method"]:checked',
                )
                .val();
                ...
                success: (res) => {
                    ...
                    $(".payment-checkout-form .list_payment_method").load(
                        window.location.href +
                            " .payment-checkout-form .list_payment_method > *",
                        function () {
                            $checkoutButton.prop("disabled", false);
                            $(document)
                                .find(
                                    '.payment-checkout-form .list_payment_method input[value="' +
    ...
    $(document)
        .on("click", ".toggle-coupon-form", () =>
            $(document).find(".coupon-form").toggle("fast"),
        )
    ...
    $(document)
        .on("click", '[data-bb-toggle="increment-room"]', (e) => {
            ...
            if (value < max) {
                $input.val(value + 1);
                $(`[data-bb-toggle="filter-${inputName}-count"]`).text(
                    value + 1,
                );
            }
        })
    ...
    // Listen for shortcode loaded events and reinitialize sliders
    document.addEventListener("shortcode.loaded", function (event) {
        const shortcodeElement = event.detail.element;
        const shortcodeName = event.detail.name;

        // Initialize sliders based on shortcode name
        switch (shortcodeName) {
            case "home-banner":
                mirandaDoc.initBannerSlider(shortcodeElement);
                break;
            case "rooms":
                mirandaDoc.initRoomSlider(shortcodeElement);
                break;
            case "testimonial":
                mirandaDoc.initTestimonialSlider(shortcodeElement);
                break;
            case "featured-news":
                mirandaDoc.initLatestPostSlider(shortcodeElement);
                break;
        }

        // Initialize common components for shortcodes with animations or popups
        const animatedShortcodes = [
            "video-introduction",
            "rooms-introduction",
            "hotel-about",
        ...
        if (animatedShortcodes.includes(shortcodeName)) {
            mirandaDoc.initWow(shortcodeElement);
        }

        // Initialize popup videos for video shortcodes
        const videoShortcodes = ["youtube-video", "video-introduction"];
        if (videoShortcodes.includes(shortcodeName)) {
            mirandaDoc.initPopupVideo(shortcodeElement);
        }

        // Initialize gallery popups
        if (shortcodeName === "all-galleries") {
            mirandaDoc.initPopupGallery(shortcodeElement);
        }
    });
})(jQuery);
